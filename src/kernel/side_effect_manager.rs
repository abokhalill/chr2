...
impl fmt::Display for EffectExecutionError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            EffectExecutionError::ExecutionFailed(msg) => {
                write!(f, "Effect execution failed: {}", msg)
            }
            EffectExecutionError::Timeout => write!(f, "Effect execution timed out"),
            EffectExecutionError::ServiceUnavailable(svc) => {
                write!(f, "Service unavailable: {}", svc)
            }
...
impl fmt::Display for AcknowledgeError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            AcknowledgeError::SubmissionFailed(msg) => {
                write!(f, "Acknowledge submission failed: {}", msg)
            }
...
pub type EffectExecutor =
    Box<dyn Fn(&EffectId, &SideEffect) -> Result<(), EffectExecutionError> + Send + Sync>;
    ...
    pub fn as_executor(&self) -> EffectExecutor {
        ...
        Box::new(move |id: &EffectId, effect: &SideEffect| {
            ...
            if should_succeed.load(Ordering::Relaxed) {
                ...
                Err(EffectExecutionError::ExecutionFailed(
    ...
    pub fn as_submitter(&self) -> AcknowledgeSubmitter {
        ...
        Box::new(move |id: EffectId| {
            ...
            if should_succeed.load(Ordering::Relaxed) {
                ...
                Err(AcknowledgeError::SubmissionFailed(
                    "Mock failure".to_string(),
                ))
            }
        })
